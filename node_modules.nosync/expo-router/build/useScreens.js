...
function fromImport(value, importedModule) {
    // [VESTURE-DEBUG] Guard: log the exact route before destructuring so we know which file is undefined
    if (importedModule == null) {
        const routeInfo = 'route=' + (value && value.route) + ' contextKey=' + (value && value.contextKey);
        console.error('[VESTURE-DEBUG] fromImport received ' + importedModule + ' for ' + routeInfo);
        throw new Error('[VESTURE-DEBUG] Route module is ' + importedModule + '. ' + routeInfo);
    }
    const { ErrorBoundary, ...component } = importedModule;
