...
function buildClosetOnlyInstructions(): string[] {
  return [
    'CLOSET-ONLY MODE: keyPieces, shoes, and accessories are arrays of real closet item ids — not descriptions. Each array\'s allowed ids are listed per tier in the WARDROBE OPTIONS block below (organised by slot: bottoms/primaryTop/secondaryTop/thermalLayer/outerwear feed keyPieces, footwear feeds shoes, watch/sunglasses/hat/bag/accessory feed accessories). Choose ids only from the tier you are building for a given recommendation — never reuse an id listed under a different tier\'s options unless it also appears under this tier\'s own list.',
    'THE OUTFIT FRAMEWORK — keyPieces must include exactly one bottoms item and exactly one primary top always. A secondary top (blazer/sport jacket, or a suit — see SUITS below) is REQUIRED for the business tier and optional for smart-casual/casual. Thermal layer and outerwear are optional and weather-dependent for every tier — include them only when the shortlist offers them and the weather/occasion calls for it, never to pad out the outfit.',
    ...
    'SUITS: a Suit id already covers BOTH the bottoms and secondary top roles at once (it is trousers + jacket in one matching piece) — if you include a Suit id in keyPieceIds, do NOT also include a separate trousers/denim/shorts item or a separate blazer/sport jacket item; the suit alone fills both. A suit is always worn with a proper collared dress shirt underneath, never a polo or t-shirt. A true weatherproof outerwear piece (overcoat) can still be added independently when the weather calls for it — it is not part of the suit\'s dual role.',
    'LAYERING OVER A STRUCTURED SECONDARY TOP: if keyPieceIds includes a blazer, sport jacket, or Suit, any additional thermal-layer piece you include must be a plain sweater/knitwear worn UNDER that jacket — never a hoodie, cardigan, or overshirt, which read as a mismatch stacked over a blazer or suit. Likewise, any additional outerwear piece must be an overcoat/trench coat able to accommodate that structured layer underneath — never a casual bomber/field/chore jacket over a blazer or suit.',
...
export function buildGenerateOutfitsInstructions(selectedTiers: OutfitTierSlug[], gender?: string | null, closetOnly?: boolean, stylistId?: StylistId) {
  return [
    ...
    'IMPORTANT — primaryVibe: classify this outfit\'s own aesthetic as exactly one primaryVibe, judged from the actual pieces/colors you chose (not the tier or occasion label) — FRESH_CLEAN (crisp, minimal, understated), WARM_COZY (soft, relaxed, comfort-forward), DARK_SEDUCTIVE (moody, sultry, night-leaning), WOODY_EARTHY (rugged, natural, outdoor-adjacent), GOURMAND_SWEET (indulgent, playful, dessert-toned palettes), AROMATIC_SPORTY (athletic, active, weekend-casual), FLORAL_ROMANTIC (soft, romantic, date-leaning), SPICY_CONFIDENT (bold, statement-making, fashion-forward). This drives which fragrance gets paired with the outfit downstream — pick the vibe that genuinely matches what you assembled, not a default.',
