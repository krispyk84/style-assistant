...
export function buildTripDayShapePrompt(
  ...
  const instructions = [
    ...
    '- formalityTier (casual | smart-casual | business): decide this EXPLICITLY per day — it is what actually constrains which real closet items are offered for that day, so get it right. Default mapping by dayType: business/meeting/wedding_event → business; conference/dinner_out/sightseeing → smart-casual; travel_day/relaxed/beach_pool/adventure → casual. BUT if ADDITIONAL USER DETAILS below gives an explicit override (e.g. "smart casual on the non-business days", "keep it casual except the gala"), that override takes priority over the default mapping for the day(s) it applies to — do not silently fall back to the default when the user has explicitly stated otherwise for that day.',
    '- contextTags: 1–4 short tags (e.g. "beach-ready", "breathable", "semi-formal", "layerable").',
    '',
    formatProfileContext(profile),
    styleGuideContext ?? 'No retrieved style-guide guidance was available for this request.',
          ...
          days: {
            ...
            items: {
              ...
              properties: {
                ...
                formalityTier: { type: 'string', enum: ['casual', 'smart-casual', 'business'], description: 'Explicit per-day formality — see RULES for the default mapping and how user overrides apply.' },
                contextTags:   { type: 'array', items: { type: 'string' }, minItems: 1, maxItems: 4 },
              },
              required: ['dayIndex', 'date', 'dayType', 'formalityTier', 'contextTags'],
              additionalProperties: false,
            },
            minItems: 1,
            maxItems: 14,
