...
export function buildClosetOutfitsChoiceSystemPrompt(): string {
  return [
    ...
    '8. LAYERING OVER A STRUCTURED SECONDARY TOP: if SECONDARY TOP is a blazer, sport coat, or Suit, a THERMAL LAYER (if you choose one) must be a plain sweater/knitwear worn UNDER that jacket — never a hoodie, cardigan, or overshirt, which are meant to be worn as a visible outer layer and would look like a mismatch stacked over a blazer or suit. Likewise, OUTERWEAR (if you choose one) must be an overcoat/trench coat able to accommodate that structured layer underneath — never a casual bomber/field/chore jacket over a blazer or suit.',
    '9. ADDITIONAL ACCESSORIES: accessoryIds is a separate, optional multi-pick list (belt/scarf/tie/socks) — include 0 or more only where they genuinely complete the look; do not pad it out for the sake of it.',
    '10. Within all of the above, look cool, current, and intentional — this is a client who cares about their aesthetic, not a rote uniform.',
    '',
